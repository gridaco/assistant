...
import { TARGET_PLATFORM, TargetPlatform } from "../../platform"
...
export abstract class MetaDataRepository<T = any> {
    ...
    fetch(): T {
        switch (TARGET_PLATFORM) {
            case TargetPlatform.figma:
                const data = figma.getNodeById(this.id).getSharedPluginData(ASSISTANT_PLUGIN_NAMESPACE, this.key)
    ...
    update(data: T) {
        switch (TARGET_PLATFORM) {
            case TargetPlatform.figma:
                figma.getNodeById(this.id).setSharedPluginData(ASSISTANT_PLUGIN_NAMESPACE, this.key, JSON.stringify(data))
                return data
            case TargetPlatform.webdev:
    ...
    clear() {
        switch (TARGET_PLATFORM) {
            case TargetPlatform.figma:
                const data = figma.getNodeById(this.id).setSharedPluginData(ASSISTANT_PLUGIN_NAMESPACE, this.key, undefined)
            case TargetPlatform.webdev:
